...
const Want = ({All , Update}) => {
  ...
  /* 
      ...
      - here we do not need all shelves we just need one shelf named = "wantToRead"
      - so we will loop on all array's items and retun only named = "wantToRead" if else, skip
      - if shelf's name = "wantToRead" return it . else,  skip
      - so now shelfCase = the shelf named = "wantToRead" which have 2 books
  */
  const shelfCase = All.filter((e)=>{
    return(e.shelf === "wantToRead" ? e : "")
  })
